...
struct BBox { double min_x, min_y, max_x, max_y; };
...
BBox ComputeBBox(const VisualizationData& data) {
    ...
    const auto expand = [&](double x, double y) {
        bb.min_x = std::min(bb.min_x, x); bb.min_y = std::min(bb.min_y, y);
        bb.max_x = std::max(bb.max_x, x); bb.max_y = std::max(bb.max_y, y);
    };
    for (const auto& poly : data.polygons)
        for (const auto& pt : poly.vertices) expand(pt.x, pt.y);
    for (const auto& pt : data.path) expand(pt.x, pt.y);
    ...
    bb.min_x -= px; bb.max_x += px;
    bb.min_y -= py; bb.max_y += py;
    return bb;
}
...
void SaveSvg(const VisualizationData& data, const std::string& filename) {
    ...
    if (data.path.size() >= 2) {
        ...
        for (const auto& pt : data.path) out << Pt(tr, pt.x, pt.y) << " ";
        out << "\"/>\n";
        // Draw path
